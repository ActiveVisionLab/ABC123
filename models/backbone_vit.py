...
class ViTExtractor(nn.Module):
    def __init__(self, config):
        ...
        base_model_ = timm.create_model(self.base_model, pretrained=self.pretrained)
    ...
    def fix_blocks(blocks, facet, layer):
        ...
        elif "token_" in facet:
            out.append(ViTExtractor.full_block_latent_output(blocks[layer], facet))
    ...
    @staticmethod
    def full_block_latent_output(block, facet):
        if "query" in facet:
            facet_idx = 0
        elif "key" in facet:
            facet_idx = 1
        ...
        class BlockWithLatentOutput(nn.Module):
            def __init__(self):
                super(BlockWithLatentOutput, self).__init__()
            ...
            def forward(self, x):
                ...
                if "query" in facet or "key" in facet or "value" in facet:
                    to_return_extra = qkv[facet_idx]
                ...
                return x, to_return_extra

        return BlockWithLatentOutput()
    ...
    def extract_descriptors(
        self, batch, layer=11, facet="key", bin=False, include_cls=False, attn_cfg=None
    ):
        """
        ...
        if "token_" in facet:
            x1, x2 = self._extract_features(batch)
            x1.unsqueeze_(dim=1)  # Bx1xtxd
            desc1 = self.format_tqkv(bin, include_cls, x1)
            if "_attn" in facet:
                desc2 = self.format_attn(attn_cfg, x2)
            else:
                desc2 = self.format_tqkv(bin, include_cls, x2)
            return desc1, desc2
            ...
            desc = self.format_tqkv(bin, include_cls, x)
            return desc

    def format_tqkv(self, bin, include_cls, x):
        if not include_cls:
    ...
    def extract_saliency_maps(
        self, img, attn_cfg={"reduction": "mean", "heads": [0, 2, 4, 5]}
    ):
        """
        ...
        return self.format_attn(attn_cfg, feat)

    def format_attn(self, attn_cfg, feat):
        reduction, heads, = attn_cfg["reduction"], attn_cfg["heads"]
    ...
    def forward(self, x):
        attn_cfg = {"reduction": None, "heads": range(6)}

        if self.facet == "attn":
            feat = self.extract_saliency_maps(x, attn_cfg)
            return feat

        elif "token_" in self.facet:
            feat1, feat2 = self.extract_descriptors(
                x, layer=self.layer, facet=self.facet, bin=self.bin, attn_cfg=attn_cfg
            )
            feat1 = feat1.squeeze(1).permute(0, 2, 1)
            B1, C1, npatch = feat1.shape
            N1 = int(math.sqrt(npatch))
            feat1 = feat1.reshape(B1, C1, N1, N1)
            if "attn" not in self.facet:
                feat2 = feat2.squeeze(1).permute(0, 2, 1)
                B2, C2, npatch = feat2.shape
                N2 = int(math.sqrt(npatch))
                feat2 = feat2.reshape(B2, C2, N2, N2)
            return feat1, feat2
